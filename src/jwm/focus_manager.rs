...
const EXPLICIT_FOCUS_MOUSE_SUPPRESSION: Duration = Duration::from_millis(300);
...
impl Jwm {
    ...
    pub(crate) fn reveal_and_focus(
        ...
        // Monitor/tag selection, arrange and deiconification can enqueue
        // EnterNotify events for a peer that was under the stationary pointer
        // before this explicit activation completed. Those events are stale:
        // letting focus-follows-mouse consume them immediately can undo a
        // successful taskbar/launcher/IPC focus request. A later real pointer
        // event is accepted once the same short guard used by keyboard focus
        // operations expires.
        let deadline = Instant::now() + EXPLICIT_FOCUS_MOUSE_SUPPRESSION;
        self.suppress_mouse_focus_until = Some(
            self.suppress_mouse_focus_until
...
mod scratchpad_reveal_tests {
    ...
    #[test]
    fn explicit_restore_focus_ignores_stale_peer_enter_then_reenables_pointer_focus() {
        let mut jwm = empty_jwm();
        jwm.add_monitor(output(1, 0));
        let monitor = jwm.state.monitor_order[0];
        let peer_window = WindowId::from_raw(0x711);
        let target_window = WindowId::from_raw(0x712);

        let mut peer = WMClient::new(peer_window);
        ...
        target.mon = Some(monitor);
        target.state.tags = 1;
        target.geometry.x = 500;
        target.geometry.y = 0;
        target.geometry.w = 700;
        target.geometry.h = 700;
        let target_key = jwm.insert_client(target);
        jwm.attach_to_monitor(target_key, monitor);
        jwm.state.monitors[monitor].set_selected_client_for_current_tag(Some(target_key));

        let mut backend = ScratchpadBackend::new();
        assert!(
            jwm.set_client_minimized(&mut backend, target_key, true)
                .unwrap()
        );
        assert_eq!(jwm.get_selected_client_key(), Some(peer_key));
        assert!(jwm.reveal_and_focus(&mut backend, target_window).unwrap());
        assert_eq!(jwm.get_selected_client_key(), Some(target_key));

        // This represents the delayed Normal EnterNotify generated by the
        // restore arrange/map sequence, not fresh pointer motion.
        jwm.handle_regular_enter_generic(&mut backend, peer_window)
            .unwrap();
        assert_eq!(jwm.get_selected_client_key(), Some(target_key));

        // Once the bounded guard expires, genuine focus-follows-mouse input
        // retains its normal behavior.
        jwm.suppress_mouse_focus_until = Some(Instant::now() - Duration::from_millis(1));
        jwm.handle_regular_enter_generic(&mut backend, peer_window)
            .unwrap();
        assert_eq!(jwm.get_selected_client_key(), Some(peer_key));
    }
