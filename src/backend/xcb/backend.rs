...
impl XcbBackend {
    ...
    fn map_client_message(&mut self, ev: x::ClientMessageEvent) -> Option<BackendEvent> {
        // A ClientMessage names whatever XID its sender wrote into it, and no
        // DestroyNotify ever evicts a window that never existed: interning
        // here would let one client grow the id table without bound. A
        // message about a window this backend has never seen cannot be acted
        // on anyway, so it is dropped instead. (The tray selection window is
        // interned when the selection is acquired.)
        let window = self.ids.existing_window(ev.window().resource_id())?;
        ...
        match classify_client_message(
            ...
            // The pinged window travels in the payload, addressed to the
            // root: resolve it the same non-allocating way.
            ClientMessageKind::PingResponse { window } => Some(BackendEvent::PingResponse {
                window: self.ids.existing_window(window)?,
            }),
...
crate::backend::x11::wm::compositor_delegation::delegate_compositor_capabilities!(
    XcbBackend,
    intern_raw = intern_raw
);

impl DisplayControl for XcbBackend {
...
impl RenderScheduler for XcbBackend {
...
mod parity_tests {
    ...
    const X11_COMPOSITOR_EXPOSE_SRC: &str = include_str!("../x11/compositor/expose.rs");
    ...
    /// This module, from its own `cfg(test)` attribute on. `XCB_BACKEND_SRC`
    /// is the file the module lives in, so a needle found in here says
    /// nothing about production code. The marker is assembled at compile
    /// time from two halves so that neither this comment nor the helper is
    ...
    fn own_test_module() -> &'static str {
        ...
        &XCB_BACKEND_SRC[start..]
    }

    /// A needle for a clause that must exist in production code, assembled
    /// at runtime by the caller and proven absent from this module before
    /// use: a written-out needle scanned over the whole of `XCB_BACKEND_SRC`
    /// finds itself at its own line, and the xcb half of the check passes no
    /// matter what the production code says.
    fn guarded_production_needle(needle: String) -> String {
        assert!(
            !own_test_module().contains(&needle),
            "needle `{needle}` appears in this test module, so a whole-file scan would prove nothing"
        );
        needle
    }
    ...
    fn x11_compositor_window_effect_overrides_stay_in_parity() {
        // The compositor capability trait impls are generated by one shared
        // macro, so parity holds by construction as long as both transports
        // invoke it instead of hand-writing impl blocks.
        let macro_call =
            guarded_production_needle(format!("{}!(", "delegate_compositor_capabilities"));
        for (label, source) in [("x11rb", X11RB_BACKEND_SRC), ("xcb", XCB_BACKEND_SRC)] {
            assert!(
                source.contains(&macro_call),
                "{label} must generate its compositor capability impls from the shared macro"
            );
            // Built at runtime so the needle does not match this test's own
            // source, which is part of XCB_BACKEND_SRC.
            let handwritten = format!("impl CompositorWindowEffects {}", "for");
            assert!(
                !source.contains(&handwritten),
                "{label} must not reintroduce a hand-written compositor capability impl"
            );
        }

        let body = impl_body_after(
    ...
    fn both_x11_backends_wire_the_iconify_coordinator_at_the_same_boundaries() {
        ...
        let field = guarded_production_needle(format!("iconify: {},", "IconifyCoordinator"));
        let init =
            guarded_production_needle(format!("iconify: {}::default(),", "IconifyCoordinator"));
        for (label, source) in [("x11rb", X11RB_BACKEND_SRC), ("xcb", XCB_BACKEND_SRC)] {
            assert!(
                source.contains(&field) && source.contains(&init),
    ...
    #[test]
    fn client_messages_resolve_windows_without_allocating_ids() {
        ...
        let xcb = impl_body_after(XCB_BACKEND_SRC, "fn map_client_message");
        assert!(
            !xcb.contains(&allocate) && xcb.contains(&resolve),
            "xcb ClientMessage translation must only resolve known windows"
        );

        let start = X11RB_BACKEND_SRC
            .find("XEvent::ClientMessage(e) =>")
            .expect("x11rb translates ClientMessage");
        let end = X11RB_BACKEND_SRC[start..]
            .find("XEvent::MappingNotify")
            .map(|offset| start + offset)
            .expect("MappingNotify follows ClientMessage in x11rb's translation");
        let x11rb = &X11RB_BACKEND_SRC[start..end];
        assert!(
            !x11rb.contains(&allocate) && x11rb.contains(&resolve),
    ...
    // The closing layer's fade gate used to be pinned here, by matching the
    // predicate's spelling in `retire_window` and `render_frame`. Both now ask
    // the single `close_fade_driven`, and the invariant moved to the module
    // that owns it: `x11::compositor::config::tests::
    // every_close_fade_site_asks_the_one_named_predicate` pins all six sites
    // and forbids a seventh from respelling it. It is not a transport-parity
    // question, so it does not belong in this module.

    #[test]
    fn thumbnail_passes_reset_the_sticky_window_uniforms() {
        // The window program's `desat` and ripple uniforms persist across
        // draws; the main pass sets them per window and leaves the last
        // window's values behind. Every pass that draws window textures
        // through that program outside the main pass must reset them before
        // its first draw, or a thumbnail's saturation depends on stacking
        // order — what the readback thumbnail already does.
        let desat = format!("uniform_1_f32(self.win_uniforms.{}.as_ref(), 0.0)", "desat");
        let ripple = format!(
            "uniform_1_f32(self.win_uniforms.{}.as_ref(), 0.0)",
            "ripple_amplitude"
        );
        for (label, src, function, first_draw) in [
            (
            ...
            (
                "render_expose",
                X11_COMPOSITOR_EXPOSE_SRC,
                "pub(super) fn render_expose",
                "for entry in &self.expose_entries",
            ),
        ] {
            let body = impl_body_after(src, function);
            let draw = body
                .find(first_draw)
                .unwrap_or_else(|| panic!("{label} must draw window textures"));
            for (name, needle) in [("desat", &desat), ("ripple_amplitude", &ripple)] {
                let reset = body
                    .find(needle.as_str())
                    .unwrap_or_else(|| panic!("{label} must reset `{name}` before drawing"));
                assert!(
                    reset < draw,
                    "{label} must reset `{name}` before its first window draw"
                );
            }
        }
    }
    ...
    fn xcb_and_x11rb_compositor_render_paths_share_lazy_tracking() {
        ...
        let xcb_after_signature = xcb_render_start + "fn compositor_render_frame".len();
        let xcb_render_end = xcb_backend_impl[xcb_after_signature..]
            .find("\n    fn ")
            .map(|idx| xcb_after_signature + idx)
        ...
        let x11rb_after_signature = x11rb_render_start + "fn compositor_render_frame".len();
        let x11rb_render_end = x11rb_backend_impl[x11rb_after_signature..]
            .find("\n    fn ")
            .map(|idx| x11rb_after_signature + idx)
