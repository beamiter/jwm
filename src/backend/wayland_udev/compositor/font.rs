...
pub(super) fn render_text_to_rgba(text: &str, scale: u32, fg: [u8; 4]) -> (Vec<u8>, u32, u32) {
    ...
    let max_cols = lines.iter().map(|l| l.len() as u32).max().unwrap_or(0);
    ...
    for (line_idx, line) in lines.iter().enumerate() {
        for (col_idx, ch) in line.chars().enumerate() {
            let glyph_index = if ch as u32 >= 32 && ch as u32 <= 126 {
                (ch as u32 - 32) as usize
            } else {
                0 // render as space for out-of-range characters
            };

            let glyph_data = &FONT_6X10[glyph_index * 10..(glyph_index + 1) * 10];

            for row in 0..GLYPH_H {
                let byte = glyph_data[row as usize];
                for col in 0..GLYPH_W {
                    // Bits are stored MSB-first in an 8-bit byte; glyph is 6 pixels wide
                    let bit = (byte >> (7 - col)) & 1;
                    if bit != 0 {
                        // Apply scale magnification
                        for sy in 0..scale {
                            for sx in 0..scale {
                                let px = col_idx as u32 * char_w + col * scale + sx;
                                let py = line_idx as u32 * char_h + row * scale + sy;
                                let offset = ((py * img_w + px) * 4) as usize;
                                pixels[offset] = fg[0];
                                pixels[offset + 1] = fg[1];
                                pixels[offset + 2] = fg[2];
                                pixels[offset + 3] = fg[3];
                            }
                        }
                    }
                }
