...
impl ShaderCache {
    ...
    /// Get a compiled shader program from cache, or compile from source.
    /// Checks memory cache first, then disk cache (binary), then compiles fresh.
        ...
        if self.enabled {
            ...
            if let Ok(data) = fs::read(&bin_path) {
                if data.len() > 4 {
                    ...
                    // Check if program binary loaded successfully
                    ...
                    // Binary failed, delete and compile from source
    ...
    /// Delete all cached programs and clear maps.
    ...
    /// Remove a shader from memory and disk cache.
    ...
    /// Hash shader source using DefaultHasher.
    ...
    /// Save the program binary to disk.
    unsafe fn save_program_binary(&self, gl: &ffi::Gles2, program: u32, name: &str) {
        ...
        // Write format (4 bytes) + binary data
        let bin_path = self.cache_dir.join(format!("{}.bin", name));
