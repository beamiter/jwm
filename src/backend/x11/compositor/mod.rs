    ...
    C: CompositorConnection,
{
    ...
    /// Classes that never play open/close effects (`behavior.fade_exclude`).
    fade_exclude: Vec<String>,
    ...
    /// Flattened EGL damage rectangles: x, bottom-left y, width, height.
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            if let Some(frame) = self.waterlily_texture.take() {
                ...
                if let Some(texture) = frame.material_texture {
                    self.gl.delete_texture(texture);
                }
            }
