...
fn theme_rules(theme: ThemeMode, background_opacity: f64) -> String {
    let palette = Palette::for_theme(theme);
    let scope = match theme {
        ThemeMode::Dark => "window.theme-dark",
        ThemeMode::Light => "window.theme-light",
    };
    let background = css(with_alpha(palette.background, background_opacity));

    format!(
        "\
{scope} .bar-root {{ background-color: {background}; }}
...
/// The complete stylesheet for one bar configuration.
#[must_use]
pub fn stylesheet(config: &PresentationConfig, font: &str, background_opacity: f64) -> String {
 ...
 * renderer paints; the frosted wallpaper sits behind its translucent tint. */
.bar-root {{
...
{light}",
        ...
        dark = theme_rules(ThemeMode::Dark, background_opacity),
        light = theme_rules(ThemeMode::Light, background_opacity),
    )
}
