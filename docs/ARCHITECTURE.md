...
本文描述 `shared_structures` 0.2 系列的共享内存协议与实现边界。安全前提和故障模型另见 [安全说明](./SAFETY.md)。
...
SharedMessage / SharedCommand        公开领域类型
              │ 转换与校验
              ▼
内部 wire 类型 + MessageSlot        固定、可验证的共享表示
              │
              ▼
GenericHeader + 两个环形队列         共享内存协议 v10
              │
              ▼
Futex / Semaphore / EventFd          等待与唤醒后端
```
...
v10 不直接打开 v9 映射。升级双方需要先停止旧进程、移除旧 flink，再共同创建 v10 映射。
...
消息槽还保存时间戳与校验和。校验和用于发现撕裂写入或内容损坏，不是密码学完整性保护。
...
| Semaphore | 进程共享 POSIX semaphore | sequence/waiter 注册握手让无人等待时跳过 `sem_post`；destroy 按已登记等待者数量唤醒，不在仍可能被其他进程引用时调用 `sem_destroy`。 |
| EventFd | Linux `eventfd` + Unix socket FD 传递 | 使用 `EFD_SEMAPHORE` 和 sequence/waiter 注册握手；创建者通过权限为 0600 的 Unix socket 传递 CLOEXEC 描述符并在 cleanup 时 join 监听线程。 |

所有后端都先执行可配置次数的自适应轮询，再进入内核等待，并以 sequence 与 waiter 计数执行 SeqCst 注册握手：signal 总会推进 sequence，但只在确有已注册 waiter 时进入内核，因此既不会丢失注册窗口中的唤醒，也不会让 Semaphore/EventFd 的轮询型消费者积累无限历史 token。通知只是优化：队列索引才是数据是否可读的事实来源。调用者被唤醒后仍需重新检查队列状态。
...
- `open_auto` 从 v10 header 自动发现后端与容量。
