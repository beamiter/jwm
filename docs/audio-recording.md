...
反馈与录屏一致：录音机真正启动后弹出开始卡片并带上输出路径，正常停止时弹出
停止卡片同样带路径；启动失败、停止失败或格式不受支持时以紧急级别 2 弹出，
可穿透免打扰——“以为在录音而实际没有”是隐私相关的情形，不能静默。

录音运行期间常驻一枚 MIC 角标：红点加静态“MIC”文字，与录屏 REC 角标同款
扁平药丸（ui.osd 底色，不做玻璃模糊）。独立录音位于 WM 侧，合成器无法像推导
REC 那样从自身采集管线得知其状态，因此由 WM 通过后端 trait
（`compositor_set_mic_indicator`）推送；标签不含计时器，每次状态变化只栅格化
一次，不需要帧泵。角标占据 REC 的位置（右下角、距边缘 16px）；录屏与独立
录音同时进行时叠在 REC 正上方，两者绝不重叠，过矮的屏幕上贴顶显示也不会
移出画面。绘制纪律与 REC 一致：X11 在帧采集之后绘制，Wayland 在投递后的
chrome 块中绘制并经 tail_domain 拦截，角标只出现在本地屏幕上，绝不泄漏进
截图或编码视频；显示期间会阻止 direct scanout（记录在案的副作用：角标可见
时独立录音会保持 exact-sRGB 回退路径）。角标只在录音机真正启动后出现——
与开始卡片同一道闸门；停止时无论文件收尾成功与否都会清除，会话拆除时同样
清除。录屏即使采集麦克风也只显示 REC 角标：MIC 是独立录音的专属提示。
