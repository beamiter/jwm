...
impl App {
    ...
    fn init_window_and_pixels(&mut self, event_loop: &EventLoop<UserEvent>) -> Result<()> {
        ...
        // delivered a depth-32 window is a separate question, and one the
        // surface cannot answer for us — wgpu reads its alpha modes from the
        // driver, never from the window's visual. A swapchain is committed
        // once, so unlike the bars that renegotiate every frame this has to be
        // right the first time: no depth answer means no alpha.
