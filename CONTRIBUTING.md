...
process group and kills that group on exit, including interrupted runs. Cleanup
never scans unrelated processes by command name. The stubborn-child tests also
use a parent-death signal and a bounded loop. It refuses to run inside a sandbox that blocks
`kill()`, where those tests fail for environmental reasons only.
`scripts/test-test-wrapper.sh` verifies cleanup and exit-status propagation with
an offline Cargo stand-in, including that an unrelated matching process survives.
